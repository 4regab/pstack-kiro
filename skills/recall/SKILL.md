---
name: recall
description: "Reconstruct recent working context from exported conversation history, current context, live state, and shared records such as prior fixes and incidents. Use for 'recall my work on X', 'catch me up', 'what have I been working on', or 'where did I leave off'."
---
...
Conversation evidence must come from an exported transcript, the current conversation context, or a path the user explicitly supplies. Never infer or probe Kiro's internal conversation storage. The shared record holds everything that happened around the same code under other names: symptoms users keep reporting, fixes that shipped and got reverted, and errors still firing in production.

1. Classify, then route. For one specific prior chat, use an exported transcript or user-provided path and summarize it directly; if neither exists, ask the user for the missing context. Turning habits into a durable skill is `automate-me`. A human-readable summary of your work is a different task. Recall loads working context across recent chats before you act. If the user already gave you a full state capsule (paths, branch, the change), use it and skip the mining.
...
3. Fan out across the available conversation evidence. Invoke named Kiro subagents concurrently through the subagent capability, one per evidence slice, and omit per-call model settings. Tell every subagent to search the topic first, read only relevant regions, and skip the current conversation plus obvious evaluation or test data. Each returns the same schema, one block per conversation: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts such as PRs, tickets, and branches, each citing the exported conversation identifier or source path. For one or two conversations, skip the fan-out and search directly. If no exported or user-provided history exists, use current context and state that broader chat history was unavailable.
...
5. Verify against live state. Conversation evidence or a stale ticket is history, not current truth, so take the PRs, branches, and tickets that the mining and sweep surfaced and check them with available source-control tools. When the answer hinges on what an agent actually did, use the full exported transcript or user-provided evidence when available; otherwise identify the gap.
