---
name: no-comments
description: Review comments and suppressions with the named comment-sicko Kiro subagent, apply only accepted in-scope findings, and offer enforceable replacements for claimed constraints. Use for no-comments or comment cleanup.
---


# No comments

Invoke the named `comment-sicko` Kiro subagent through the subagent capability. Act on accepted findings. If that named agent is unavailable, stop and ask the user to configure or provide it; do not substitute an invented review policy.

Authoring agents defend comments. Defer to the `comment-sicko` agent's fresh perspective.
...
1. Invoke the named `comment-sicko` Kiro subagent through the subagent capability and pass the scope. Omit per-call model settings. Do not restate its rules. If the named agent is unavailable, require the user to provide or configure it before continuing.
