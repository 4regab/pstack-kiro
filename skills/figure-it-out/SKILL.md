...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with isolated, opinionated candidates and an independent named Kiro judge instructed not to modify files. Use separately configured model families only when those named agents already exist; otherwise disclose the single-model fallback. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
