...
2. **Source wave.** Invoke one named Kiro subagent per feature file concurrently through the subagent capability. Omit per-call model settings and instruct each child not to modify files. Each explains "how does this user-facing feature work?" from source, flags likely doc drift with citations, and returns one concise live-verification recipe. Children never drive the app and never edit files. Return shape: feature summary / source entry points / likely drift or none / one recipe.
