...
4. **Run N parallel candidates** through Kiro's subagent capability. Use named custom agents when distinct prompts, tools, permissions, or model configurations are part of the experiment. Each candidate works in its own sanitized directory with the same prompt.
5. **Run one blinded judge** as a separate named agent or subagent. The judge sees outputs by sanitized label and the rubric, never an agent or model name.
6. **Verify the chain from explicit artifacts, not self-report.** Use the current conversation context when the run is still present. Otherwise use an IDE chat export or a CLI `/chat save` artifact that the operator explicitly supplied. Do not infer or scan internal chat storage. Look at which files each candidate actually opened when the artifact records that evidence. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the recorded actions plus the shape of the code, never from the candidate's own claims.
