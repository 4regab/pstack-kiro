...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <exported transcript path>", "you're taking over", "pick up where X left off", a Kiro cloud session handoff, or a pushed branch you're meant to continue.
...
1. Locate the prior trail. Prefer the current Kiro context when the work is still present. Otherwise use an IDE chat export, a CLI `/chat save` artifact that the operator explicitly supplied, a Kiro cloud session, a repository handoff note, or a pushed branch. Do not infer or scan Kiro's internal chat storage. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long artifact in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
