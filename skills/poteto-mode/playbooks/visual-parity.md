...
4. Verify each component against its baseline via image diff on the matching surface with the UI or project verification tools actually available. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Iterate within a stated attempt budget and checkpoint each result.
