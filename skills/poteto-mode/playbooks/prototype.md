...
5. Verify on the matching surface. For a visual decision, screenshot each variant with the available project or Kiro UI tools and drive the interaction when browser, simulator, or MCP tooling exists; the eye is the test. For a behavioral or timing decision, observe the decision by logging the timing, printing the output, or watching the render. The observation is the test here, not an assertion.
