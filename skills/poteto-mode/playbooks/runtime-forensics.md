...
1. Capture the live signal with the project or Kiro runtime tools actually available: a CPU profile for a spinning process, a heap snapshot for a leak, or a trace for a visual glitch. A real artifact, not a guess.
2. Reduce the artifact to the smoking gun: the function on the hot path, the retainer chain from the leaked object to a GC root, or the repeated operation firing without input. Parse large artifacts in a subagent (the **guard-the-context-window** principle skill), keep the reduced finding in the main thread.
