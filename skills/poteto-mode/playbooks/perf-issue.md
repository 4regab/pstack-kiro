...
1. Capture a baseline trace with the available project or Kiro runtime tools.
...
3. Plan the fix from the trace. If it crosses a function boundary, run the available design agent or fan out design review first. Delegate implementation to a named agent or native subagent with the selected/default model; review the diff. Capture a post-fix trace.
