...
> **Kiro Crew port of [`cursor/plugins/pstack`](https://github.com/cursor/plugins/tree/main/pstack)** by [poteto (Lauren Tan)](https://x.com/poteto).
> Skills, agents, automations, and docs converted 1:1 to Kiro Crew's format so you can run poteto's rigorous engineering workflow on Kiro.
>
> **What changed in the port:**
> - `.cursor-plugin/plugin.json` → `.kiro-plugin/plugin.json`
> - Each `skills/<name>/SKILL.md` keeps its `name`/`description`; Kiro `triggers:` are derived from the skill slug, and Cursor's `disable-model-invocation: true` skills become manual-only (`/slug`).
> - Each `agents/<name>.md` → a Kiro agent config `agents/<name>.json` + system prompt `agents/<name>.prompt.md`.
> - `automations/` (benny), `docs/`, `LICENSE` preserved as-is.
>
> All credit for the original design and content goes to poteto. Original text below.

## Install (Kiro Crew)

Copy the `skills/` directory into `~/.kiro/crew/skills/` (each subdirectory is one skill), drop the `agents/*.json` configs into `~/.kiro/agents/`, and restart your Kiro Crew gateway. Skills load on startup; agents appear on the dashboard Agents page.
